...
import 'package:soundify/view/pages/container/primary/add_song_container.dart';
import 'package:soundify/view/pages/container/primary/bottom_container.dart';
...
import 'package:soundify/view/pages/container/primary/liked_song_container.dart';
...
import 'package:soundify/view/pages/widget/play_list.dart';
...
class _MainPageState extends State<MainPage> {
  ...
  void initState() {
    ...
    FirebaseFirestore.instance
        ...
        .snapshots()
        .listen((snapshot) {
      final newPlaylists = snapshot.docs.map((doc) {
        return {
          'creatorId':
              doc['creatorId'] ?? '', // Provide a default empty string if null
          ...
          'timestamp': (doc['timestamp'] as Timestamp?)?.toDate() ??
              DateTime.now(), // Handle possible null timestamp
          'playlistUserIndex':
              doc['playlistUserIndex'] ?? 0, // Default to 0 if null
          'songListIds':
              doc['songListIds'] ?? [], // Default to an empty list if null
          'totalDuration': doc['totalDuration'] ?? 0,
        };
      }).toList();

      // Sort the playlists by playlistUserIndex in descending order
      newPlaylists.sort(
          (a, b) => b['playlistUserIndex'].compareTo(a['playlistUserIndex']));

      // Add sorted data to the StreamController
      _playlistsController.add(newPlaylists);
    });
  }
  ...
  Future<void> _logout() async {
    await FirebaseAuth.instance.signOut(); // Logout dari akun Firebase
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
          builder: (context) =>
              const LoginPage()), // Navigasi ke LoginPage setelah logout
    );
  }
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: Column(
                                                      ...
                                                      children: [
                                                        ...
                                                        TextButton(
                                                          onPressed:
                                                              _logout, // Panggil fungsi logout saat tombol diklik
                                                          child: const Text(
                                                            "Log Out",
                                                            style: TextStyle(
                                                              color: Colors
